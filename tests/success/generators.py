...
assert assertEqual(compileGen(fieldFooClass, fields=frozenset(
    {"foo"})), CLASS("foo", Field("foo", useClasses=False)))
...
assert assertEqual(compileGen(presentQuestion),
                   Literal("Question is present in the model"))
assert assertEqual(compileGen(presentAbsent), emptyGen)
assert assertEqual(compileGen(absentQuestion), emptyGen)
assert assertEqual(compileGen(absentAbsent),
                   Literal("Absent is absent in the model"))

assert assertEqual(compileGen(
    asked, asked=frozenset({"asked"})), Literal("is asked"))
...
assert assertEqual(compileGen(notAsked, asked=frozenset({})),
                   Literal("is not asked"))
assert assertEqual(compileGen(notAsked, asked=frozenset({}),
                              hide=frozenset({"asked"})), emptyGen)
...
assert assertEqual(compileGen(cascadeUseless, asked=frozenset()),
                   Literal("Asked is not asked"))
assert assertEqual(compileGen(cascadeUseless, asked=frozenset(
    ["cascaded"])), Literal("Asked is not asked"))
assert assertEqual(compileGen(cascadeUseless, asked=frozenset(
    ["asked"])), Literal("Asked is asked"))
assert assertEqual(compileGen(cascadeUseless, hide=frozenset(
    ["cascaded"])), Literal("Asked is not asked"))
assert assertEqual(compileGen(
    cascadeUseless, hide=frozenset(["asked"])), emptyGen)
assert assertEqual(compileGen(cascade, asked=frozenset()),
                   Literal("Cascaded is not asked"))
assert assertEqual(compileGen(cascade, asked=frozenset(
    ["cascaded"])), Literal("Cascaded is asked"))
assert assertEqual(compileGen(cascade, asked=frozenset(
    ["asked"])), Literal("Cascaded is asked"))
...
assert assertEqual(listEmptyInexistingField.force(),
                   listEmptyInexistingField_.force())
...
assert assertEqual(listEmptyInexistingField.force(),
                   listEmptyInexistingFieldNormal.force())
assert assertEqual(listEmptyInexistingField.force(),
                   listEmptyInexistingFieldWR.force())
assert assertEqual(compileGen(listEmptyInexistingField), literalFoo)

assert assertEqual(compileGen(listEmptyExistingField),
                   ListElement([literalFoo, fieldQuestion]))
...
assert assertEqual(compileGen(questionsRecursive,
                              isQuestion=False), Literal("answer side"))
...
assert assertEqual(compileGen(presentOrAbsentQuestion),
                   Literal("Question is present in the model"))
assert assertEqual(compileGen(presentOrAbsentAbsent),
                   Literal("Absent is absent from the model"))

### Asked or Not
assert assertEqual(compileGen(
    askedOrNotTest, asked=frozenset({"askedOrNot"})), Literal("Asked"))
assert assertEqual(compileGen(
    askedOrNotTest, asked=frozenset()), Literal(text="notAsked", ))
...
assert assertEqual(compileGen(twoQuestionsAsTable,
                              asked=frozenset({"Definitions"}),
                              fields=frozenset(
                                  {"Definition", "Definition2", "Name", "Name2"}),
                              isQuestion=False),
